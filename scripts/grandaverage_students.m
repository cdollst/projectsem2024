...
%that all numbers are equal, they should no longer be 90 or 10
   ...
   %Equalize the number of trials
    for nc = 1 : length(epochtrans2) %per person, you want to know out of all of the trials of the epoch that wasn't deleted, how many correspond to the current conditions, there should be more common epochs than rare epochs
        trial_indices=find(strcmpi(EpochNames,epochtrans2{nc}));%for each condition in each subject, it finds the indices of relevant trials using 'find'

        if length(trial_indices)>target_n_trials %if the number of indices of relevant trials exceeds the target, it randomly samples a subset using 'randsample'
            %Randomly select the trials to keep
            trial_indices = randsample(trial_indices, target_n_trials);
        elseif length(trial_indices) < target_n_trials
            warning(['Subject ' num2str(subjects(i)) ' has fewer than ' num2str(target_n_trials) ' trials for condition ' epochtrans2{nc}]);

            %Ensure the subject has the minimum acceptable number of trials
            %across all conditions, if the number of trials is less than
            %the target, a warning is issued
            if any(n_trials(i,:)<target_n_trials)
                error(['Subject ' num2str(subjects(i)) ' does not have enough trials for one or more conditions, adjust "target_n_trials" accordingly.']);
            end
        end
        ...
        % subject, this dataset is equalized and has the GrandAverage
        GrandAverageEEG(i,nc,:,:) = mean(EEG.data(:,:,trial_indices),3);
        n_trials(i,nc) = length(trial_indices); % Store the number of trials
    end
end 

%Check that all conditions have an equal number of trials
disp('Number of trials per condition for each subject:');
disp(n_trials);
