...
figure;
subplot(2,2,1)
elec = EEG.chanlocs(1:62); %tell matlab which electrodes to use to plot because with the variable we created above, from the topo structure, it's just numbers, so use the EEG channel location structure, so that matlab knows what data goes where
