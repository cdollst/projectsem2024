...
for id = subjects
    ...
    EEG = pop_eegfiltnew(EEG, 'locutoff', ,'hicutoff', ,'plotfreqz',1);
...
for id = subjects
    ...
    pathName = '';
    EEG = pop_loadset(strcat(id{1}, '_resampled_filtered.set'), pathName);
    EEG = pop_reref( EEG, ,'keepref','');
...
%% 3. Visual Inspection to Check & Deal with bad channels
